# 230227 - ENEL 693 Assignment 3 - UnitCommitment.py - Austyn Nagribianko
...
def main():
    ...
    model.cons = ConstraintList() # Initializing a constraint list to add each constraint
    ...
    for i in range(1, hours):
        model.cons.add(
            expr=model.y1[i] - model.z1[i] == model.u1[i] - model.u1[i - 1])  # Unit 1 Start-Up Shut-Down Equality
        model.cons.add(
            expr=model.y2[i] - model.z2[i] == model.u2[i] - model.u2[i - 1])  # Unit 2 Start-Up Shut-Down Equality
    ...
    # Ramp Up Limits
    model.cons.add(expr=model.p1[0] - p1_0 <= power["rampUp"][0] * u1_0 + power["startUp"][0] * model.y1[
        0])  # Unit 1 Ramp-Up Limits
    model.cons.add(expr=model.p2[0] - p2_0 <= power["rampUp"][1] * u2_0 + power["startUp"][1] * model.y2[
        0])  # Unit 2 Ramp-Up Limits
    for i in range(1, hours):
        model.cons.add(
            expr=model.p1[i] - model.p1[i - 1] <= power["rampUp"][0] * model.u1[i - 1] + power["startUp"][0] * model.y1[
                i])  # Unit 1 Ramp-Up Limits
        model.cons.add(
            expr=model.p2[i] - model.p2[i - 1] <= power["rampUp"][1] * model.u2[i - 1] + power["startUp"][1] * model.y2[
                i])  # Unit 2 Ramp-Up Limits

    # Ramp Down Limits
    model.cons.add(expr=p1_0 - model.p1[0] <= power["rampDown"][0] * model.u1[0] + power["shutDown"][0] * model.z1[
        0])  # Unit 1 Ramp-Down Limits
    model.cons.add(expr=p2_0 - model.p2[0] <= power["rampDown"][1] * model.u2[0] + power["shutDown"][1] * model.z2[
        0])  # Unit 2 Ramp-Down Limits
    for i in range(1, hours):
        model.cons.add(
            expr=model.p1[i - 1] - model.p1[i] <= power["rampDown"][0] * model.u1[i] + power["shutDown"][0] * model.z1[
                i])  # Unit 1 Ramp-Down Limits
        model.cons.add(
            expr=model.p2[i - 1] - model.p2[i] <= power["rampDown"][1] * model.u2[i] + power["shutDown"][1] * model.z2[
                i])  # Unit 2 Ramp-Down Limits

    # Hourly Reserve Requirements
    for i in range(hours):
        model.cons.add(
            expr=demand["load"][i] + demand["reserve"][i] <= model.u1[i] * power["max"][0] + model.u2[i] * power["max"][
                1])  # Hourly Reserve Requirement
    ...
    for i in range(hours):
        ...
        model.cons.add(expr=demand["load"][i] == model.p13[i] + model.p23[i])  # Bus 3 Flow Balance
    ...
    model.obj = Objective(expr=
                          sum(cost["fixed"][0] * model.u1[i] + cost["variable"][0] * model.p1[i] + cost["startUp"][0] *
                              model.y1[i] + cost["shutDown"][0] * model.z1[i] +
                              cost["fixed"][1] * model.u2[i] + cost["variable"][1] * model.p2[i] + cost["startUp"][1] *
                              model.y2[i] + cost["shutDown"][1] * model.z2[i] for i in range(hours)))
    ...
    temp = 0
    for i in range(hours):
        temp += cost["fixed"][0] * model.u1[i]() + cost["variable"][0] * model.p1[i]() + cost["startUp"][0] * model.y1[
            i]() + \
                cost["shutDown"][0] * model.z1[i]() + cost["fixed"][1] * model.u2[i]() + cost["variable"][1] * model.p2[
                    i]() + \
